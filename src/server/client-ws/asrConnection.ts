 ...
 */
const ASR_WS_ENDPOINT = process.env.ASR_WS_ENDPOINT?.trim() || "ws://192.168.3.228:9000/v1/streaming";
...
export const initializeAsrConnection = (socket: Socket) => {
  ...
  asrSocket.on("message", (rawData) => {
    ...
    const { type, is_final, text, requestId, echoTimestamp } = parsedPayload || {};
    if( type === "result") {
       console.log(text, 'asr 结果')
      if(is_final === true) {
